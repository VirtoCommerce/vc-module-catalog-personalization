...
namespace VirtoCommerce.CatalogPersonalizationModule.Test.Indexations
{
    ...
    public class TaggedItemIndexChangesProviderTests
    {
        ...
        protected ITaggedItemSearchService GetTaggedItemSearchService()
        {
            ...
            service.Setup(x => x.SearchTaggedItems(It.IsAny<TaggedItemSearchCriteria>()))
                .Returns<TaggedItemSearchCriteria>((criteria) => new GenericSearchResult<TaggedItem> { Results = _allTaggedItems.Where(p => criteria.Ids.Contains(p.Id)).ToArray() });
